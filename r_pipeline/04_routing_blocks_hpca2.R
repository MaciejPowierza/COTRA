...
run_block_hpca <- function(X, blocks, ncomp_per_block = 2, scale. = TRUE, ncomp_per_block_adjusting = TRUE) {
  ...
  # --- hPCA via direct RGCCA call (replaces multiblock::hpca) ---
  # multiblock::hpca is a thin wrapper around RGCCA::rgcca with:
  #   - super-block = cbind of centered blocks
  #   - connection: super-block connected to all blocks, blocks not interconnected
  #   - tau = 1 for regular blocks (standardized), tau = 0 for super-block
  #   - scheme = function(x) x^4 (4th-order)
  n_block <- length(block_list_2)

  # Center each block (scale = FALSE, just centering — matching multiblock::hpca)
  X_hpca <- lapply(block_list_2, function(M) scale(as.matrix(M), scale = FALSE))

  # Super-block = concatenation of all centered blocks
  X_hpca[[n_block + 1]] <- do.call(cbind, X_hpca)

  # Connection matrix: super-block (last) connected to all blocks
  C_hpca <- matrix(0, n_block + 1, n_block + 1)
  C_hpca[n_block + 1, 1:n_block] <- 1
  C_hpca[1:n_block, n_block + 1] <- 1

  # ncomp vector: same k for all blocks + super-block
  ncomp_hpca <- rep(k_hpca, n_block + 1)

  # Names for RGCCA blocks
  names(X_hpca) <- c(names(block_list_2), "superblock")
  rownames(C_hpca) <- colnames(C_hpca) <- names(X_hpca)

  res_hpca <- RGCCA::rgcca(
    blocks       = X_hpca,
    connection   = C_hpca,
    tau          = c(rep(1, n_block), 0),
    scale        = scale.,
    ncomp        = ncomp_hpca,
  ...
  # Super-scores (common components): X_super %*% astar_super
  X_pca <- as.matrix(X_hpca[[n_block + 1]] %*% res_hpca$astar[[n_block + 1]])

  # Block-specific scores: X_b %*% astar_b
  X_blocks <- list()
  for (j in seq_len(n_block)) {
    Sj <- as.matrix(X_hpca[[j]] %*% res_hpca$astar[[j]])
    if (ncol(Sj) > 0) {
      X_blocks[[names(block_list_2)[j]]] <- Sj
    }
  }

  # Store model for compatibility with downstream code
  hpca_model <- res_hpca
